...
void ColumnVariant::validateState() const
{
    ...
    if (local_discriminators_data.size() != offsets_data.size())
        throw Exception(ErrorCodes::INCORRECT_DATA, "Size of discriminators and offsets should be equal, but {} and {} were given", local_discriminators_data.size(), offsets_data.size());
    ...
    for (size_t i = 0; i != local_discriminators_data.size(); ++i)
    {
        ...
        if (local_discr != NULL_DISCRIMINATOR)
        {
            ...
            if (offsets_data[i] >= actual_variant_sizes[local_discr])
                throw Exception(ErrorCodes::INCORRECT_DATA, "Offset at position {} is {}, but variant {} ({}) has size {}", i, offsets_data[i], static_cast<UInt32>(local_discr), variants[local_discr]->getName(), variants[local_discr]->size());
        }
    }
    ...
    for (size_t i = 0; i != variants.size(); ++i)
    {
        if (variants[i]->size() != expected_variant_sizes[i])
            throw Exception(ErrorCodes::INCORRECT_DATA, "Variant {} ({}) has size {}, but expected {}", i, variants[i]->getName(), variants[i]->size(), expected_variant_sizes[i]);
    }
}
